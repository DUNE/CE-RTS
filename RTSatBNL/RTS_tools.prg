...
Function JumpToSocket(DAT_nr As Integer, socket_nr As Integer)
	If DAT_nr = 2 Then
		Jump P(200 + socket_nr) :Z(-130)
		Print P(200 + socket_nr)
	ElseIf DAT_nr = 1 Then
		Jump P(100 + socket_nr) :Z(-130)
...
Function isChipInSocket(DAT_nr As Integer, socket_nr As Integer) As Boolean
	...
	If DAT_nr = 1 Then
		Go Here -Z(15)
		'need to be tuned according to calibration result. Please run "Socket_height_calibration"
	ElseIf DAT_nr = 2 Then
		Go Here -Z(15)
		'need to be tuned according to calibration result. Please run "Socket_height_calibration"
	Else
			Print "DAT number ", DAT_nr, " wrong, should be 1 or 2."
	EndIf
...
Function PickupFromSocket(DAT_nr As Integer) As Boolean
	...
	Wait 0.5
	If Not isContactSensorTouches Then
		Print "***ERROR! Do not detect chip in the socket, try again"
		Go Here -Z(1)
		If Not isContactSensorTouches Then
			Print "***ERROR! Do not detect chip in the socket, give up"
			Exit Function
		EndIf
	EndIf
	Wait 1
...
Function JumpToSocket_camera(DAT_nr As Integer, socket_nr As Integer)
	...
	ElseIf DAT_nr = 2 And socket_nr > 19 Then
		Jump P(200 + socket_nr) :Z(-97.60) +Y(DF_CAMERA_OFFSET) +U(0)
	ElseIf DAT_nr = 1 And socket_nr < 19 Then
		Jump P(100 + socket_nr) :Z(-97.60) +X(DF_CAMERA_OFFSET) +U(135)
	ElseIf DAT_nr = 1 And socket_nr > 19 Then
		Jump P(100 + socket_nr) :Z(-97.60) +X(DF_CAMERA_OFFSET) +U(90)
	EndIf
Fend
...
Function ChipBottomAnaly(id$ As String, ByRef idx() As Integer, ByRef res() As Double) As Integer
	...
	'Print "XN: ChipBottomAnaly start: ", Here
	Integer i, fileNum
	
	ChipBottomAnaly = 0
	
	'global var for position correction, initial 0
	X_COR = 0
	Y_COR = 0
	A_COR = 0
	...
	ElseIf tgt_pallet_nr > 0 And tgt_pallet_nr > NTRAYS And tgt_col_nr > 0 And tgt_col_nr <= TRAY_CD_NCOLS And tgt_row_nr > 0 And tgt_row_nr <= TRAY_CD_NROWS Then
		tgt_x0 = tray_CD_X(tgt_pallet_nr, tgt_col_nr, tgt_row_nr)
		tgt_y0 = tray_CD_Y(tgt_pallet_nr, tgt_col_nr, tgt_row_nr)
		tgt_u0 = tray_CD_U(tgt_pallet_nr, tgt_col_nr, tgt_row_nr)
		dst_U = CU(Pallet(tgt_pallet_nr, tgt_col_nr, tgt_row_nr))
	Else
		ChipBottomAnaly = 100
		Exit Function
	...
	'Print "XN: 1st VRun ChipBottom_Analy: ", Here
	'Add angle test
	'Go Here +U(2)
	...
	Print #fileNum, ",", pict_fname$,
	'Print "XN: 2nd VRun ChipBottom_Analy: ", Here
	VRun ChipBottom_Analy
	...
	'Print "XN: 3rd VRun ChipBottom_Analy: ", Here
	VRun ChipBottom_Analy
	...
	'pict_fname$ = UF_take_picture$(id$ + "_02")
	'Print #fileNum, ",", pict_fname$,
	'Print "XN: 4th VRun ChipBottom_Analy: ", Here
	VRun ChipBottom_Analy
	...
	'Print "XN: 5th VRun ChipBottom_Analy: ", Here

	VRun ChipBottom_Analy
	Print "XN1: X_0 Y_0 U_0: ", X_0, Y_0, U_0
	VGet ChipBottom_Analy.Final.Found, ret_found
	If ret_found Then
	...
	EndIf
	Print "XN2: X_0 Y_0 U_0: ", X_0, Y_0, U_0
	...
	d_X = X_0 - X_tool
	d_Y = Y_0 - Y_tool
	If Abs(d_X) < 5 And Abs(d_Y) < 5 And Abs(U_0) < 5 Then
	...
	EndIf
	Print "XN::Check after correction : X_0 Y_0 U_0: ", X_0, Y_0, U_0
	...
	Print "XN: key and blob check: ", Here
...
Function MoveChipFromTrayToSocket(pallet_nr As Integer, col_nr As Integer, row_nr As Integer, DAT_nr As Integer, socket_nr As Integer) As Int64
	...
	''This is for ColDATA oritation detection based on OCR
	'If pallet_nr > 10 Then
	
	''run ocr for direction check:
	'Wait 2
	'If OCR Then
	'	Print "direction right"
	'Else
'		RTS_error(fileNum, "direction wrong")
'        MoveChipFromTrayToSocket = -7
'		Exit Function
'	EndIf
'	EndIf
	...
	EndIf
	'XNtest
	'Print "XN: Jump to Tray ", Here

	' Take picture of the bottom of the chip
	JumpToCamera
	'Print "XN: Jump to Camera ", Here
	
	Integer status
	'ChipBottomAnaly(chip_SN$, ByRef idx(), ByRef status, ByRef res())
	If pallet_nr > 10 Then
		status = ChipBottomAnaly_CD(ts$, ByRef idx(), ByRef res())
	Else
...
Function MoveChipFromSocketToTray(DAT_nr As Integer, socket_nr As Integer, pallet_nr As Integer, col_nr As Integer, row_nr As Integer) As Int64
	...
	'Go Here -X(0.8)
	'Go Here +U(0.8)
	'If pallet_nr = 11 Then
	'	Go Here +U(90)
	'EndIf

	If Not PickupFromSocket(DAT_nr) Then
